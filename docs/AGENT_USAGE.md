...
2. 新锐企划任务选择 `xinrui-main`；其他作品或账号选择 `creator-default`。
3. 在“超级智能体”中写清作品目标、平台、时长、预算和已有剧本。
4. 需要关键帧时勾选生图，并保留“付费生成前暂停审批”和“视觉终审后继续”。
5. 点击“开始执行”，在右侧按步骤查看项目、剧本审查、资源清单、研究、提示词、故事板、关键帧、视频计划、宣发和归档结果。
6. 到达付费生图闸门时，确认模型、预算和提示词后再批准；图片出现后先做角色、服装、道具、手部、空间、轴线和连续性检查。
7. 任务完成后，从 `output/projects/<project-slug>/` 获取分阶段文件；不要直接改写原始资料库。

明确创作指令会创建真实的持久任务，而不是只返回一份计划。关闭页面或重启服务后，任务记录、步骤、审批、错误和产物仍会保留。
