const ILLUSTRATION_STANDARD = "xinrui-key-scene-illustrations-v1";
...
function inferIllustrationIntent(shot, index) {
  ...
  if (/倒计时|无人机转向|转向声|突然熄灭/.test(text)) {
    return {
      title: "无声倒计时",
      visualFocus: "门缝光点熄灭后的瞬间，林荫清抬手让队伍进入无声倒计时",
      shotType: "广角中近景 / 悬疑收束",
      characterPerformance: "林荫清抬手制止声音，队员进入预备动作，表情克制紧张",
  ...
  if (/门禁|维修日志|通讯包|异常通讯/.test(text)) {
    return {
      title: "门禁异常通讯包",
      visualFocus: "门禁终端上伪装成维修日志的异常通讯包，被林荫清发现",
      shotType: "中近景 / 信息发现",
      characterPerformance: "林荫清靠近终端，手指停在屏幕前，另一只手示意队伍停下",
      environment: "门禁面板、暗色墙面、雨夜楼道反光，队员在背景压低动作",
      lighting: "终端冷光照亮林荫清侧脸和手部，背景压暗",
  ...
  if (/终端|线路|无人机|巡逻轨迹|接入/.test(text)) {
    return {
      title: "终端接入",
      visualFocus: "唐舒嫣在侧门边接入终端，屏幕显示敌方无人机巡逻轨迹",
      shotType: "中景 / 科技操作",
      characterPerformance: "唐舒嫣半蹲操作终端，林荫清在近处警戒或位于背景形成保护关系",
  ...
  if (/连廊|雨夜|积水|反光|楼顶/.test(text)) {
    return {
      title: "雨夜连廊潜入",
      visualFocus: "林荫清带队穿过雨夜楼顶连廊，手电光扫过积水中的红色反光",
      shotType: "广角建立 / 雨夜潜入",
      characterPerformance: "林荫清在队伍前方低姿推进，战术手电压低，队员保持距离",
  ...
  if (/门后|人质|诱导信号源|诱导信号/.test(text)) {
    return {
      title: "诱导信号源确认",
      visualFocus: "林荫清借手电反射确认门后不是人质，而是诱导信号源",
      shotType: "中近景 / 战术判断",
      characterPerformance: "林荫清压低枪口，身体贴近门侧，视线集中在反射光和门缝细节",
  ...
  if (/两人|交换.*眼神|绕到?侧门|吸引.*监听|主门/.test(text)) {
    return {
      title: "默契分工",
      visualFocus: "林荫清留在主门前吸引监听，唐舒嫣绕向侧门，两人用眼神完成分工",
      shotType: "中景 / 双人调度",
      characterPerformance: "林荫清正面或侧身守主门，唐舒嫣从侧后方移动，两人短暂对视",
      environment: "主门与侧门同时可读，走廊墙角、门框和暗部形成空间方向",
      lighting: "林荫清一侧由手电边缘光勾出，唐舒嫣由终端或门缝冷光补亮",
  ...
  if (/门缝|光点|信号|通讯/.test(text)) {
    return {
      title: "异常光点判断",
      visualFocus: "门缝下的冷色通讯光点、林荫清的手电光束和警觉视线",
      shotType: "中近景 / 低照度悬疑",
      characterPerformance: "林荫清半蹲或侧身停步，手电斜切门缝，表情冷静但警觉",
  ...
  if (/停止|抬手|压低|轴线/.test(text)) {
    return {
      title: "停止破门",
      visualFocus: "林荫清抬手制止队伍，破门前一秒的静止张力",
      shotType: "广角中景 / 战术调度",
      characterPerformance: "林荫清位于画面主导位置，一手持枪或手电，一手做停止手势",
  ...
  if (/重新|布置|绕到?侧门|换位|队形/.test(text)) {
    return {
      title: "无声换位",
      visualFocus: "林荫清指挥唐舒嫣绕侧门，队形在走廊内重新展开",
      shotType: "中景 / 队形移动",
      characterPerformance: "林荫清压低动作发出手势，唐舒嫣从侧后方向侧门移动",
  ...
  if (/潜入|走廊|楼层|进入/.test(text)) {
    return {
      title: "楼内潜入建立",
      visualFocus: "林荫清带队进入停电楼层，手电光扫过潮湿地面和门廊",
      shotType: "广角建立 / 潜入氛围",
      characterPerformance: "林荫清在队伍前方，低姿推进，队员保持距离",
  ...
  if (/眼神|冷静|停在/.test(text)) {
    return {
      title: "冷静收束",
      visualFocus: "林荫清的眼神与门缝光点形成对照，暗示下一步行动",
      shotType: "近景 / 情绪钉子",
      characterPerformance: "林荫清侧脸或三分之二脸，眼神冷静，队友在背景完成换位",
